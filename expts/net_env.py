...
class NetworkEnv(object):
    """
    Network Environment for Discovery,Added negative penalty

    :param fullGraph: actual graph
    :param seeds: list of initial seed nodes
    :param max_T: max length of episode
    :param influence_algo: function that inputs the discovered graph and does influence maximization to give reward
    """
    ...
    def step(self, action):
        ...
        '''
        if action == -1: # 如果 action 为 -1，则停止探索，返回当前的影响力奖励。
            self.done = True
    ...
    def state(self):
        ...
        return nx.to_numpy_array(self.sub)
