...
class GraphTD3(nn.Module):
    ...
    def get_embeddings(self, nodes_attr, adj, nodes=None, adjust_dim=True, convert_torch=True):
        if convert_torch:
            ...
            # 转换为 torch.Tensor
            nodes_attr = torch.from_numpy(nodes_attr).to(self.device)
            adj = torch.from_numpy(adj).to(self.device)

        state_embed = self.graph_embedder.forward(nodes_attr, adj)[:, -self.graph_embedding_dim:] # 就是state dim，保留最后的embedding_dim
        action_embed = None
...
class DQNTrainer:
    ...
    def get_values2(self, nodes_attr, adj, action):
        ...
        a = torch.from_numpy(np.copy(action.reshape((1,) + action.shape))).to(self.device)
        q1, q2 = self.actor_critic.critic_forward(np.array([s[0]], dtype=np.float32),
                                                  np.array([s[1]], dtype=np.float32),
                                                  nodes=a,
                                                  adjust_dim=False,
                                                  node_labels=False)
        return q1.cpu().data.numpy().ravel()[0], q2.cpu().data.numpy().ravel()[0]
    ...
    def save_models(self, filename, path='models'):
        ...
        torch.save({
            'models': self.actor_critic.cpu().state_dict(),
        }, path)
    ...
    def load_models(self, filename, path='models'):
        ...
        utls.copy_parameters(self.target_actor_critic, self.actor_critic)
        print("models loaded from %s" % path)
