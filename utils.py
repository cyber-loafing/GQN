...
def greedy(items, budget, f, init=[]) -> tuple[set, float]:
    """
    在指定预算内选择项目子集，以最大化给定的目标函数。

    该函数采用贪心算法，并使用边际增益的惰性评估，适用于目标函数 f 为次模（submodular）的情况。该函数
    迭代选择项目，基于它们对整体目标的估计贡献，直到达到预算限制。
    :param items List[int]，图的序号（比如采集100个活跃边图）
    :param budget 指的是IM中的预算
    :param f 目标函数
    :param init 初始项目子集
    :return S: 选择的项目子集, starting_objective: 选择的项目子集的目标函数值
    """
    import heapq
